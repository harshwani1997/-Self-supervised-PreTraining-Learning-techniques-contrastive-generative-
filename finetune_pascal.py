...
class SWaVFineTunerPascalSeg(pl.LightningModule):
    ...
    def shared_step(self, batch):
        ...
        with torch.no_grad():
            features = self.backbone.model.forward_backbone(x)
...
def cli_main():
    ...
    parser.add_argument('--backbone_dir', type=str, help='path to dataset', default='./weights')
    ...
    backbone_path = os.path.join(args.backbone_dir, f'epoch={args.backbone_checkpoint}.ckpt')
    print(f"Backbone = {backbone_path}")
    backbone = SwAV.load_from_checkpoint(backbone_path, strict=False)
    backbone.logmodel.eval_mode = True

    model = SWaVFineTunerPascalSeg(
        backbone,
